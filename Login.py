...
BLACKLIST_PROCESSOS = [
    "idaq.exe", "idaq64.exe", "x64dbg.exe", "x32dbg.exe", "ollydbg.exe", "gdb.exe", "windbg.exe", "edb.exe",
    "dnspy.exe", "ghidra.exe", "cutter.exe", "binaryninja.exe", "malcat.exe", "reclass.net.exe",
    "cheatengine-x86_64.exe", "cheatengine-i386.exe", "charles.exe", "fiddler.exe", "wireshark.exe",
    "procmon.exe", "processhacker.exe", "httpdebuggerui.exe", "tcpview.exe"
]
...
def tela_de_login_servidor():
    ...
    try:
        ...
        if response.status_code in [200, 201] and resposta_json.get("status") == "sucesso":
            ...
            # --- CÓDIGO DE TEMPO RESTANTE REMOVIDO ---
            time.sleep(4)
